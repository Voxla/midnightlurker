	...
	public static boolean execute(LevelAccessor world, double x, double y, double z) {
		...
		lurker = new File((FMLPaths.GAMEDIR.get().toString() + "/config/"), File.separator + "midnightlurkerconfig.json");
		{
			try {
				...
				if (mainjsonobject.get("pig_shapeshifter_spawning").getAsBoolean() == true) {
				...
				} else if (mainjsonobject.get("pig_shapeshifter_spawning").getAsBoolean() == false) {
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("midnightlurker:lurkerdisappear")), SoundSource.NEUTRAL, 0, 0);
						} else {
		...
		if (mainjsonobject.get("pig_shapeshifter_spawning").getAsBoolean() == true) {
			if (!world.getEntitiesOfClass(Pig.class, AABB.ofSize(new Vec3(x, y, z), 400, 400, 400), e -> true).isEmpty()
					&& !(!world.getEntitiesOfClass(ShapeshifterPigEntity.class, AABB.ofSize(new Vec3(x, y, z), 400, 400, 400), e -> true).isEmpty())) {
				return true;
			}
		} else if (mainjsonobject.get("pig_shapeshifter_spawning").getAsBoolean() == false) {
			return false;
